...
import { AdmissionRegistration, DEFAULT_GRADE_FEES, DEFAULT_SETTINGS, DEFAULT_TRANSPORTATION_AREAS, GRADE_LEVELS, GradeFeeSettings, SchoolSettings, TransportationArea } from '../models/admission.models';
...
export class StorageService {
  ...
  private readonly settingsState = signal<SchoolSettings>(this.normalizeSettings(DEFAULT_SETTINGS));
  ...
  readonly pendingCount = computed(() => this.registrations().filter((item) => item.status === 'pending').length);
  ...
  saveDraft(registration: AdmissionRegistration): void {
    localStorage.setItem(DRAFT_KEY, JSON.stringify({ ...registration, updatedAt: new Date().toISOString() }));
  }
  ...
  upsertRegistration(registration: AdmissionRegistration): AdmissionRegistration {
    ...
    const registrations = exists ? existing.map((item) => (item.id === next.id ? next : item)) : [next, ...existing];
    this.registrationsState.set(registrations);
    return next;
  }

  deleteRegistration(id: string): void {
    const registrations = this.registrationsState().filter((item) => item.id !== id);
    this.registrationsState.set(registrations);
  }
  ...
  notify(message: string, targetRoles: UserRole[] | 'all' = 'all', category: AppNotification['category'] = 'admin', link?: string, sourceId?: string): void {
    if (sourceId && this.notificationsState().some((item) => item.sourceId === sourceId)) return;
    ...
    const notifications = [notification, ...this.notificationsState()].slice(0, 50);
    this.notificationsState.set(notifications);
  }
  ...
  notificationsFor(role?: UserRole): AppNotification[] {
    if (!role) return [];
    return this.notificationsState().filter((item) => item.targetRoles === 'all' || item.targetRoles.includes(role));
  }

  unreadNotificationsFor(role?: UserRole): number {
    if (!role) return 0;
    return this.notificationsFor(role).filter((item) => !item.readBy.includes(role)).length;
  }

  markNotificationsRead(role?: UserRole): void {
    if (!role) return;
    const notifications = this.notificationsState().map((item) => {
      const visible = item.targetRoles === 'all' || item.targetRoles.includes(role);
  ...
  ensureNotification(message: string, targetRoles: UserRole[] | 'all', category: AppNotification['category'], link: string, sourceId: string): void {
    this.notify(message, targetRoles, category, link, sourceId);
  }

  hasDuplicate(field: 'nationalId' | 'passportNumber' | 'phone' | 'email', value: string, ignoreId?: string): boolean {
    ...
    return this.registrationsState().some((registration) => {
      ...
      if (field === 'phone') return values[2] === normalized || values[3] === normalized;
  ...
  private normalizeSettings(settings: SchoolSettings): SchoolSettings {
    ...
    const transportationAreas = (settings.transportationAreas?.length ? settings.transportationAreas : DEFAULT_TRANSPORTATION_AREAS).map((area, index) => this.normalizeTransportationArea(area, index));
    return {
      ...DEFAULT_SETTINGS,
      ...settings,
      fees: {
        ...
        ...currentFees,
        transportationFee: fees['transportationFee'] ?? oldTransportationFee ?? DEFAULT_SETTINGS.fees['transportationFee']
      },
      gradeFees,
      transportationAreas,
  ...
  private normalizeTransportationArea(area: Partial<TransportationArea>, index: number): TransportationArea {
    const name = area.name?.trim() || `Area ${index + 1}`;
    return {
      id: area.id || name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || crypto.randomUUID(),
      name,
