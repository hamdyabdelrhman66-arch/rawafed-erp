...
export class FinanceStorageService {
  ...
  private backendAccountToPackage(account: any): FinancePackage {
    ...
    return {
      ...
      grossFees: Number(account.grossFees ?? account.expectedTotal ?? account.total ?? 0),
      totalDiscounts: Number(account.totalDiscounts || 0),
      netFees: Number(account.netFees ?? account.total ?? 0),
      ...
      originalPlanAmount: Number(account.originalPlanAmount || account.expectedTotal || 0),
      discountAmount: Number(account.discountAmount || 0),
      netPlanAmount: Number(account.netPlanAmount || account.netFees || account.total || 0),
      hasOutstandingWithoutInvoice: Boolean(account.hasOutstandingWithoutInvoice),
  ...
  private backendInvoiceToInvoice(invoice: any): FinanceInvoice {
    return {
      ...
      additionalDiscount: Number(invoice.additionalDiscount || 0),
      grossTotal: Number(invoice.total || 0),
      vat: Number(invoice.vat || 0),
      total: Number(invoice.netTotal ?? invoice.total ?? 0),
      date: (
        invoice.issuedAt ||
        invoice.createdAt ||
        new Date().toISOString()
      ...
      studentDiscounts: invoice.studentDiscounts || [],
