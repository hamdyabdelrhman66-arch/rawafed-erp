...
import { FinanceData, FinanceExpense, FinanceInvoice, FinancePackage, FinancePayment, FinanceStudent, registrationToFinanceStudent } from './finance.models';
...
export class FinanceStorageService {
  ...
  getPatient(id: number): Observable<FinanceStudent | undefined> {
    return this.getPatients().pipe(map((items) => items.find((item) => item.id === Number(id))));
  }
  ...
  updatePatient(id: number, patient: FinanceStudent): Observable<FinanceStudent> {
    const data = this.read();
    const next = { ...patient, id: Number(id) };
    this.write({ ...data, patients: data.patients.map((item) => (item.id === Number(id) ? next : item)) });
    return of(next);
  }
  ...
  getPackages(): Observable<FinancePackage[]> {
    return from(this.api.get<any[]>('/finance/accounts')).pipe(
      map((accounts) => accounts.map((account) => this.backendAccountToPackage(account)))
    );
  }

  getPackage(id: number): Observable<FinancePackage | undefined> {
    return this.getPackages().pipe(map((items) => items.find((item) => item.id === Number(id))));
  }

  addPackage(studentPackage: FinancePackage): Observable<FinancePackage> {
    const data = this.read();
    const next = { ...studentPackage, id: Number(studentPackage.id || Date.now()) };
    this.write({ ...data, packages: [...data.packages, next] });
    return of(next);
  }

  updatePackage(id: number, studentPackage: FinancePackage): Observable<FinancePackage> {
    const data = this.read();
    const next = { ...studentPackage, id: Number(id) };
    this.write({ ...data, packages: data.packages.map((item) => (item.id === Number(id) ? next : item)) });
    return of(next);
  }

  deletePackage(id: number): Observable<void> {
    const data = this.read();
    this.write({ ...data, packages: data.packages.filter((item) => item.id !== Number(id)) });
    return of(void 0);
  }

  getPayments(): Observable<FinancePayment[]> {
    return from(this.api.get<any[]>('/finance/payments')).pipe(
      map((payments) => payments.map((payment) => this.backendPaymentToPayment(payment)))
    );
  }

  getPayment(id: number): Observable<FinancePayment | undefined> {
    return this.getPayments().pipe(map((items) => items.find((item) => item.id === Number(id))));
  }

  addPayment(payment: FinancePayment): Observable<FinancePayment> {
    if (payment.accountId) {
      return from(this.api.post<any>('/finance/payments', {
  ...
  updatePayment(id: number, payment: FinancePayment): Observable<FinancePayment> {
    const data = this.read();
    const next = { ...payment, id: Number(id) };
    this.write({ ...data, payments: data.payments.map((item) => (item.id === Number(id) ? next : item)) });
    return of(next);
  }

  deletePayment(id: number): Observable<void> {
    const data = this.read();
    this.write({ ...data, payments: data.payments.filter((item) => item.id !== Number(id)) });
    return of(void 0);
  }

  getInvoices(): Observable<FinanceInvoice[]> {
    return from(this.api.get<any[]>('/finance/invoices')).pipe(
      map((invoices) => invoices.map((invoice) => this.backendInvoiceToInvoice(invoice)))
    );
  }

  getInvoice(id: number): Observable<FinanceInvoice | undefined> {
    return this.getInvoices().pipe(map((items) => items.find((item) => item.id === Number(id))));
  }
  ...
  updateInvoice(id: number, invoice: FinanceInvoice): Observable<FinanceInvoice> {
    const data = this.read();
    const next = { ...invoice, id: Number(id) };
    this.write({ ...data, invoices: data.invoices.map((item) => (item.id === Number(id) ? next : item)) });
    return of(next);
  }

  deleteInvoice(id: number): Observable<void> {
    const data = this.read();
    this.write({ ...data, invoices: data.invoices.filter((item) => item.id !== Number(id)) });
    return of(void 0);
  }

  getExpenses(): Observable<FinanceExpense[]> {
    return from(this.api.get<any[]>('/finance/expenses')).pipe(
      map((expenses) => expenses.map((expense) => this.backendExpenseToExpense(expense)))
    );
  }

  getExpense(id: number): Observable<FinanceExpense | undefined> {
    return this.getExpenses().pipe(map((items) => items.find((item) => item.id === Number(id))));
  }
  ...
  updateExpense(id: number, expense: FinanceExpense): Observable<FinanceExpense> {
    const data = this.read();
    const next = { ...expense, id: Number(id) };
    this.write({ ...data, expenses: data.expenses.map((item) => (item.id === Number(id) ? next : item)) });
    return of(next);
  }

  deleteExpense(id: number): Observable<void> {
    const data = this.read();
    this.write({ ...data, expenses: data.expenses.filter((item) => item.id !== Number(id)) });
    return of(void 0);
  }

  ensureAccountFromRegistration(registration: AdmissionRegistration): FinancePackage {
    const data = this.read();
    const existingPackage = data.packages.find((item) =>
    ...
    this.write({
      ...data,
      patients: data.patients.some((item) =>
        item.registrationId === registration.id ||
        (!item.registrationId && item.registrationNumber === registration.registrationNumber)
      )
        ? data.patients
  ...
  ensureAccountsFromRegistrations(registrations: AdmissionRegistration[]): FinancePackage[] {
    registrations
      .filter((registration) => registration.submittedAt || registration.status !== 'draft')
      .forEach((registration) => this.ensureAccountFromRegistration(registration));
  ...
  private read(): FinanceData {
    try {
      ...
      return this.normalize(raw ? (JSON.parse(raw) as Partial<FinanceData>) : EMPTY_FINANCE_DATA);
    } catch {
  ...
  private write(data: FinanceData): void {
    localStorage.setItem(FINANCE_STORAGE_KEY, JSON.stringify(this.normalize(data)));
    window.dispatchEvent(new CustomEvent(FINANCE_UPDATED_EVENT));
  }
  ...
  private backendAccountToPackage(account: any): FinancePackage {
    ...
    return {
      ...
      status: account.status === 'paid' ? 'Paid' : account.status === 'partial' ? 'Partial' : 'Unpaid',
  ...
  private backendInvoiceToInvoice(invoice: any): FinanceInvoice {
    return {
      ...
      date: (invoice.issuedAt || invoice.createdAt || new Date().toISOString()).slice(0, 10),
  ...
  private financeId(registration: AdmissionRegistration, suffix: number): number {
  ...
  private buildFeeItems(registration: AdmissionRegistration): FinancePackage['services'] {
    const financial = registration.financial;
    const items = [
      { service: 'Registration Fee', price: Number(financial.registrationFee || 0) },
    ...
    if (financial.transportationRequired) {
      items.push({ service: 'Bus Transportation', price: Number(financial.transportationFee || 0) });
    }
