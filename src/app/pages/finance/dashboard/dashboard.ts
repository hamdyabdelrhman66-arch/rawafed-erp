...
export class Dashboard implements OnInit {
  ...
  loadDashboard() {
    forkJoin({
      ...
      this.totalRevenue = Number(accounting?.kpis?.revenue || 0);

      const today = new Date().toISOString().split("T")[0];

      this.todayCollection = this.payments
        .filter((p: any) => p.date === today)
        .reduce((sum: number, p: any) => sum + Number(p.amount), 0);

      this.outstanding = Number(accounting?.kpis?.accountsReceivable || 0);

      this.activePackages = this.packages.length;

      this.unpaidPackages = this.packages.filter(
        (x) => x.status === "Unpaid",
      ).length;

      this.partialPackages = this.packages.filter(
        (x) => x.status === "Partial",
      ).length;

      this.recentPayments = [...this.payments].reverse().slice(0, 5);

      this.recentInvoices = [...this.invoices].reverse().slice(0, 5);

      this.topOutstanding = [...this.packages]
  ...
  get kpiCards(): any[] {
    ...
    return [
      {
        label: this.i18n.t("finance.kpi.total_revenue"),
        value: this.totalRevenue,
        suffix: this.i18n.t("currency.SAR"),
      ...
      },
      {
        ...
        value: this.todayCollection,
        suffix: this.i18n.t("currency.SAR"),
        note: this.i18n.t("finance.kpi.collections_today_note"),
        icon: "today",
        tone: "neutral",
        route: "/finance/payments",
      },
      {
        label: this.i18n.t("finance.kpi.outstanding_receivables"),
        value: Number(kpis.accountsReceivable || 0),
        suffix: this.i18n.t("currency.SAR"),
        note: this.i18n.t("finance.kpi.outstanding_receivables_note"),
        icon: "account_balance_wallet",
        tone: "warning",
        route: "/finance/patient-packages",
      },
      {
        label: this.i18n.t("finance.kpi.accounts_payable"),
        value: Number(kpis.accountsPayable || 0),
        suffix: this.i18n.t("currency.SAR"),
      ...
      },
      {
        ...
        value: Number(kpis.cashBalance || 0),
        suffix: this.i18n.t("currency.SAR"),
        note: this.i18n.t("finance.kpi.cash_balance_note"),
        icon: "point_of_sale",
        tone: "neutral",
        route: "/finance/cashboxes",
      },
      {
        label: this.i18n.t("finance.kpi.bank_balance"),
        value: Number(kpis.bankBalance || 0),
        suffix: this.i18n.t("currency.SAR"),
      ...
      },
      {
        ...
        value: Number(kpis.netProfit || 0),
        suffix: this.i18n.t("currency.SAR"),
        note: this.i18n.t("finance.kpi.net_profit_note"),
        icon: "trending_up",
        tone: Number(kpis.netProfit || 0) >= 0 ? "positive" : "negative",
        route: "/finance/accounting",
      },
      {
        label: this.i18n.t("nav.student_accounts"),
        value: this.activePackages,
        suffix: "",
        note: this.i18n.t("finance.kpi.student_accounts_note", {
          unpaid: this.unpaidPackages,
          partial: this.partialPackages,
        }),
        icon: "school",
        tone: "neutral",
        route: "/finance/patient-packages",
  ...
  loadAlerts() {
    ...
    this.packages.forEach((pkg: any) => {
      ...
      const endDate = new Date(pkg.expectedEndDate);
