...
export class ChartOfAccounts implements OnInit {
  ...
  printLedger(): void {
    ...
    popup.document.write(`<html><head><title>Ledger ${details.code}</title><style>@font-face{font-family:Cairo;src:url('/fonts/Cairo.ttf') format('truetype');font-weight:400 800;font-style:normal}body{font-family:Cairo,sans-serif;padding:30px;color:#14233b}h1{color:#123d73}table{width:100%;border-collapse:collapse}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left}th{background:#123d73;color:white}</style></head><body><h1>${details.code} · ${details.nameEn}</h1><p>${details.nameAr || ''}</p><p>Opening ${this.money(details.openingBalance)} · Current ${this.money(details.currentBalance)} · Debit ${this.money(details.debitTotal)} · Credit ${this.money(details.creditTotal)}</p><table><thead><tr><th>Date</th><th>Entry</th><th>Description</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead><tbody>${rows}</tbody></table></body></html>`);
