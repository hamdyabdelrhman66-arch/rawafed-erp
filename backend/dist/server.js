...
import { z } from 'zod';
import { hashToken, requireAuth, requireRole, signRefreshToken, signUser, verifyRefreshToken } from './auth.js';
...
app.post('/api/auth/refresh', (req, res) => {
    ...
    try {
        ...
        const db = readDb();
        const stored = db.refreshTokens.find((item) => item.id === decoded.tokenId &&
...
app.post('/api/auth/logout', requireAuth, (req, res) => {
    ...
    updateDb((db) => {
        ...
        db.refreshTokens = db.refreshTokens.map((item) => {
            if (body.success && body.data.refreshToken && item.tokenHash !== hashToken(body.data.refreshToken))
                return item;
...
app.post('/api/users', requireAuth, requireRole(['Super Admin']), async (req, res) => {
    ...
    let created;
    updateDb((db) => {
        if (db.users.some((item) => item.username === body.data.username))
            return;
        created = {
            id: randomUUID(),
...
app.patch('/api/users/:id', requireAuth, requireRole(['Super Admin']), (req, res) => {
    const body = z.object({
...
app.patch('/api/users/:id/password', requireAuth, requireRole(['Super Admin']), (req, res) => {
...
app.patch('/api/users/:id/status', requireAuth, requireRole(['Super Admin']), (req, res) => {
...
app.post('/api/registrations', requireAuth, requireRole(['Admissions', 'Registrar']), (req, res) => {
...
app.patch('/api/registrations/:id/status', requireAuth, requireRole(['Admissions', 'Registrar', 'Principal']), (req, res) => {
    ...
    updateDb((db) => {
        ...
        if (updated && status.data === 'approved')
            createStudentFromRegistrationRecord(db, updated);
    });
    if (!updated) {
    ...
    if (status.data === 'approved') {
        logAudit({ actorId: req.user?.id, actorRole: req.user?.role, action: 'approve application', entityType: 'registration', entityId: updated.id });
    }
    res.json(updated);
});
...
app.post('/api/students/from-registration/:registrationId', requireAuth, requireRole(['Admissions', 'Registrar', 'Principal']), (req, res) => {
    let student;
    updateDb((db) => {
...
app.patch('/api/students/:id', requireAuth, requireRole(['Admissions', 'Registrar', 'Principal']), (req, res) => {
...
app.delete('/api/students/:id', requireAuth, requireRole(['Super Admin']), (req, res) => {
    updateDb((db) => {
...
app.post('/api/notifications/:id/read', requireAuth, (req, res) => {
    updateDb((db) => {
        ...
        if (note && req.user?.role && !note.readBy.includes(req.user.role))
            note.readBy.push(req.user.role);
    });
    res.status(204).send();
});
...
app.post('/api/finance/payments', requireAuth, requireRole(['Finance']), (req, res) => {
    const body = z.object({
        accountId: z.string(),
...
app.post('/api/finance/invoices', requireAuth, requireRole(['Finance']), (req, res) => {
    ...
    const invoice = {
        ...
        status: body.data.status === 'Paid' ? 'Paid' : 'Pending',
        issuedAt: String(body.data.date || now),
        createdAt: now
    };
    updateDb((db) => db.financeInvoices.unshift(invoice));
    logAudit({ actorId: req.user?.id, actorRole: req.user?.role, action: 'create invoice', entityType: 'finance_invoice', entityId: invoice.id });
...
app.put('/api/settings', requireAuth, requireRole(['Super Admin']), (req, res) => {
    ...
    logAudit({ actorId: req.user?.id, actorRole: req.user?.role, action: 'change settings', entityType: 'settings', entityId: 'settings' });
    res.json(readDb().settings);
});
app.listen(port, () => {
...
function createStudentFromRegistrationRecord(db, registration) {
